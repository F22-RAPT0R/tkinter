...
class EntryRadioCheck(tk.Frame):
    def __init__(self, master, textvariable, num, id, on_entry_focus, on_entry_enter):
        # on_entry_focus: method from master to be executed when Entry widget takes focus
        # on_entry_enter: method from master to be executed when user presses entery key in Entry widget
        self.master=master
        self.num=num
        self.textvariable=textvariable
        self.id=id
        self.on_entry_focus=on_entry_focus
        self.on_entry_enter=on_entry_enter
        self._seperator=","
    ...
    def _layout(self):
        ...
        self.entry=tk.Entry(self, textvariable=self.entry_textvariable)
        self.entry.grid(row=0, column=1, sticky="we", padx=2)
        self.entry.bind("<FocusIn>", lambda event:self.on_entry_focus(self.id))
        self.entry.bind("<Tab>", lambda event:self.on_entry_enter(event, self.id))    # when user pressed enter key in Entry
...
class App(tk.Tk):
    def __init__(self, num, title="ANKI", *args, **kwargs):
        ...
        self.num=num
        self._last_row=0
        self.entryradiochecks={}    # EntryRadioChecks in app, key is EntryRadioCheck's id
    ...
    def _add_EntryRadioCheck(self):
            ...
            erc=EntryRadioCheck(master=self, textvariable=entryradiocheck_variable, num=9, id=self._last_row, on_entry_focus=self.on_entry_focus, on_entry_enter=self.on_entry_enter)
            self.entryradiochecks[self._last_row]=erc
            erc.grid(row=self._last_row, column=0, sticky="nswe", padx=10, pady=5)

    def on_entry_focus(self, entry_id):
        if entry_id==self._last_row:
            self._add_EntryRadioCheck()

    def on_entry_enter(self, event, id):
        # when user presses enter key in a Entry widget of a EntryRadioCheck, Entry of next row's EntryRadioCheck should take focus 
        # print(event.__dict__)
        self.entryradiochecks[id+1].entry.focus_set()
        return "break"  # by default, Tab changes focus, to keep it from doing that, add return "break" at the end of your callback.

    def _debug1(self, msg):
        print(msg)
